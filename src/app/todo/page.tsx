...
export default function TodoPage() {
  ...
  const { isPanelOpen, openPanel, closePanel } = useSceneUI("todo");
  ...
  return (
    <AppShell tab="todo" activeGoal={activeGoal} userCoreCoach={coreCoach}>
      ...
      <ScenePanel open={isPanelOpen} onClose={closePanel} title="Todo Panel">
              ...
              <Panel>
                <PanelHeader>
                  <h2 className="text-lg font-extrabold text-gray-900">
                    {activeGoal?.goalType === "structured" ? "백로그 추가(나중)" : "투두 추가"}
                  </h2>
                      ...
                      {activeTodosByActiveGoal.map((todo) => (
                        <li key={todo.id} className="flex items-center justify-between rounded-xl border border-gray-200 px-3 py-2">
                          ...
                          <div className="ml-2 flex items-center gap-1">
                            {editingTodoId === todo.id ? (
                              <>
                                <button type="button" onClick={saveEditTodo} className="ui-btn-secondary">저장</button>
                                <button type="button" onClick={cancelEditTodo} className="ui-btn-secondary">취소</button>
                              </>
                            ) : (
                              <>
